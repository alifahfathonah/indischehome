...
class LoginForm extends CFormModel
{
	...
	public function authenticate($attribute,$params)
	{
		if(!$this->hasErrors())
		{
			...
			/*print_r($this->_identity->ERROR_USER_NOT_ACTIVE);
			exit();*/
			if($this->_identity->authenticate() == 1 || $this->_identity->authenticate() == 2)
				$this->addError('password','Incorrect username or password.');
			else if($this->_identity->authenticate() == 3)
				$this->addError('password', 'User is not active, please ask IndischeHome administrator to give privileges to your account.');
		}
	}
